...
| 2 | Browser → `POST /administrator/api/uploads/sign` | Body: `{ galleryId }`. El endpoint: valida `Astro.locals.profile`, comprueba vía `ContentRepository` que ese perfil **puede editar** esa galería (mismo predicado que RLS), y responde `{ timestamp, signature, apiKey, cloudName, folder: "lead-utp/<area-slug>/<gallery-slug>", uploadPreset }`. |
| 3 | Browser → `POST https://api.cloudinary.com/v1_1/<cloud>/image/upload` | Una petición por archivo, con los params firmados. Cloudinary responde `{ public_id, secure_url, width, height, format, bytes }`. |
...
- Cloudinary aplica: `allowed_formats=jpg,png,webp`, `max_file_size` (p. ej. 10 MB), auto-`webp`/`avif` y `quality:auto` en la **entrega** (no en el original).
