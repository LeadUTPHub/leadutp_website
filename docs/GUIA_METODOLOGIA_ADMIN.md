...
> **Nota — fase actual del proyecto (validación 100% local):** el repo todavía no está conectado a Vercel. Mientras eso no pase, `git push origin cms-admin` solo respalda el trabajo en GitHub, no genera ninguna URL de preview. La validación de cada sprint, por ahora, se hace enteramente en tu máquina con `pnpm dev` (para revisar en caliente) y `pnpm build && pnpm preview` (para confirmar que el build de producción funciona igual que en un entorno real). Cuando varios sprints ya estén estables en local, conecta Vercel siguiendo los pasos de la sección anterior — recién ahí este flujo de preview por PR entra en juego, antes del merge final a `main`.
...
Cuando el sprint esté listo, corre tú mismo `pnpm build && pnpm preview`
y dime el resultado — Vercel todavía no está conectado, así que la
validación de este sprint es 100% local. Recuérdame además hacer:
  git push origin cms-admin
para respaldar el trabajo en GitHub (esto no genera preview todavía,
solo backup remoto).
```
 
**Tu rol en cada sprint:** correr `pnpm dev` y `pnpm build && pnpm preview` en tu máquina, probar exactamente lo que promete el DoD de ese sprint (ni más ni menos), y solo entonces decir "aprobado, sigue con el Sprint N+1". Si algo del sprint no cumple su DoD, se queda ahí hasta que se resuelva — no se avanza con deuda pendiente. Cuando conectes Vercel más adelante, este mismo rol se traslada a probar en la preview URL en vez de (o además de) local.
